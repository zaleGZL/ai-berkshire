...
- **修改 `reports/` 下已有的报告（`community/` 除外）、`筛选公司/`**：这些是我个人的研究产出，不接受外部修改。你自己的研究请提交到 `reports/community/`（见上），或在自己的仓库发布后在 issue 里分享链接。
...
- **Edits to existing reports under `reports/` (except `community/`) or `筛选公司/`** — these are my personal research output. Submit your own research to `reports/community/` (see above), or publish it in your own repo and share the link in an issue.
